...
class Fish(RotatedImage):
    ...
    def set_game_screen(self, game_screen):
        """Установить ссылку на игровой экран"""
    ...
    def new_fish(self, *args):
        ...
        # Размещаем рыбу в центре экрана
        self.center = self.GAME_SCREEN.center
        self.opacity = 1
        self.interaction_block = False

    def swim(self):
        # Рыба остается в центре, только появляется с анимацией
        self.center = self.GAME_SCREEN.center
        self.opacity = 0
        appear = Animation(opacity=1, duration=0.5)
        appear.start(self)
        appear.bind(on_complete=lambda w, a: setattr(self, "interaction_block", False))

    # Перемогли рибу :)
    def defeated(self):
        self.interaction_block = True

        # Запоминаем центр для возврата
        old_center = self.center

        # Анімація обертання
        anim = Animation(angle=self.angle + 360, d=1, t='in_cubic')

        # Запам'ятовуємо старі розмір
        old_size = self.size.copy()
        # Новий розмір
        new_size = (self.size[0] * self.COEF_MULT * 3, self.size[1] * self.COEF_MULT * 3)

        # АНІМАЦІЯ ЗБІЛЬШЕННЯ/ЗМЕНШЕННЯ с сохранением центра
        anim &= Animation(size=(new_size), t='in_out_bounce') + Animation(size=(old_size), duration=0)
        # Возвращаем центр после изменения размера
        anim &= Animation(opacity=0)
        anim.start(self)

        # Возвращаем рыбу в центр после анимации
        def reset_center(*args):
            self.center = old_center

        anim.bind(on_complete=reset_center)
    ...
    def on_touch_down(self, touch):
        ...
        if not self.anim_play and not self.interaction_block:
            ...
            # Показываем всплывающий текст с очками
            self.GAME_SCREEN.show_click_points(click_points, touch.pos)
            ...
            if self.hp_current > 0:
                ...
                # Возвращаем центр после анимации
                def reset_center(*args):
                    self.center = old_center
                    self.anim_play = False

                zoom_anim.bind(on_complete=reset_center)
...
class Game(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.fish_widget.set_game_screen(self)  # Устанавливаем ссылку на игровой экран
        ...
        # Счетчик очков за клик
        self.click_bonus_label = Label(
            text="+1 за клик",
            size_hint=(None, None),
            size=(dp(150), dp(30)),
        ...
        main_layout.add_widget(self.click_bonus_label)
    ...
    def update_score_label(self, instance, value):
        ...
        # Обновляем счетчик бонуса за клик
        total_bonus = 1  # Базовое значение
        for upgrade_key, upgrade_data in app.UPGRADES.items():
            total_bonus += upgrade_data['level'] * upgrade_data['bonus_per_level']
        self.click_bonus_label.text = f"+{total_bonus} за клик"

    def show_click_points(self, points, pos):
        """Показать всплывающий текст с очками"""
        points_label = Label(
            text=f"+{points}",
            size_hint=(None, None),
            size=(dp(60), dp(30)),
            pos=(pos[0] - dp(30), pos[1]),
            font_size=sp(20),
            bold=True,
            color=(1, 1, 0, 1)
        )
        ...
        anim = Animation(y=pos[1] + dp(50), opacity=0, duration=0.8)
        anim.start(points_label)

        # Удаляем метку после анимации
        def remove_label(*args):
            self.remove_widget(points_label)
